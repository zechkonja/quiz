...
import {updateNumberOfPlayers, updateGameCategory, updateQuestionNumber} from "../actions/gameSettings";
...
const HomeGameSettings = () => {
  ...
  const gameCategories = useSelector((state) => state.gameSettings.gameCategories);
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Form.Select onChange={handlePlayerChange}
                     value={playerNumber}>
        ...
        <Form.Select onChange={handleCategoryChange}
                     value={playerNumber}>
          {gameCategories.map(cat => <option key={cat.id} value={cat.id}>{cat.name}</option>)}
        </Form.Select>
        <br />
        <label className={'form-label'}>Number of Questions:</label>
        <input type="number" min="1" max="50" onKeyUp={handleNumberQuestionChange}/>
        <br />
